...
func main() {
	...
	go func() {
		...
		if err != nil {
			log.Printf("watcher consumer: %v", err)
			return
		}
		_, err = cons.Consume(func(msg jetstream.Msg) {
			log.Printf("COUNTER_GLOBAL msg: subject=%s nats-incr=%s body=%s",
				msg.Subject(), msg.Headers().Get("Nats-Incr"), string(msg.Data()))
			msg.Ack()
		})
		if err != nil {
			log.Printf("watcher consume: %v", err)
		}
	}()
